# app.py — Buffett & Quant Copilot (sticky panels + API hardening + Buffett image)
...
# Global style: sticky header panel
st.markdown("""
<style>
.sticky-panel {
...
.sticky-panel .block-container { padding-top: 0 !important; }
</style>
""", unsafe_allow_html=True)

# ─────────────────────────────────────────────────────────────
# Images & fallback (Buffett image applied)
# ─────────────────────────────────────────────────────────────
HERO_IMG_PATH = "/mnt/data/32d37cf6-cb03-4500-ab21-08ee05047b34.png"   # 헤더용(없으면 URL)
HERO_IMG_FALLBACK = "https://images.unsplash.com/photo-1559526324-593bc073d938?q=80&w=1600"

# ★ 사이드바 로고: 버핏 이미지로 고정 우선
BUFFETT_LOGO_PATH = "/mnt/data/90c0cdf6-11dd-4eb5-8cf1-e194862434d7.png"  # 제공된 버핏 이미지
...
with col_key1:
    test_now = st.button("🔍 연결 테스트", help="가벼운 호출로 키 유효성을 확인합니다.")
with col_key2:
    file_logo = st.file_uploader("사이드바 로고(선택)", type=["png","jpg","jpeg"], key="sidebar_logo_uploader")
    if file_logo is not None:
        st.session_state["sidebar_logo_bytes"] = file_logo.read()
...
if looks_like_key(openai_api_key):
    try:
        ...
        if "invalid_api_key" in msg or "Incorrect API key" in msg or "401" in msg:
            ...
            st.error("요청 중 오류가 발생했습니다: " + redact(msg))
            st.stop()
else:
    st.warning("입력한 값이 API 키 형식처럼 보이지 않습니다. `sk-...` 형태를 확인하세요.")
    st.stop()
...
def stream_chat(messages: List[Dict[str, str]]):
    with st.chat_message("assistant"):
        ...
        try:
            ...
            placeholder.error("요청 중 오류가 발생했습니다: " + redact(str(e)))
            return ""
...
# Buffett (sticky panel + output container)
with tabBuffett:
...
# Quant (sticky panel + output container)
with tabQuant:
    st.markdown('<div class="sticky-panel">', unsafe_allow_html=True)
